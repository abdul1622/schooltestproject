...
class SubjectSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if queryset.filter(name=data['name'], grade=data['grade']).exists():
            raise serializers.ValidationError(
                {'error': 'subject name altready exists'})
        if queryset.filter(code=code).exists():
            raise serializers.ValidationError(
                {'error': 'subject code altready exists'})
...
class ChapterSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Chapter
        fields = ['id', 'subject', 'subject_name', 'name',
                  'chapter_no', 'description', 'created_at']
...
class QuestionGetSerializer2(serializers.ModelSerializer):
    ...
    from_chapter = serializers.PrimaryKeyRelatedField(
